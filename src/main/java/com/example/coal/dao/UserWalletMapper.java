...
//    添加新的钱包信息（添加新用户的时候，直接进行钱包的添加【user_id,wallet_state】）
    int addNewWallet(int user_id,int wallet_state,String userName,String BcardNum);
//    更改钱包金额信息/银行卡信息（记得同时更新账单信息）
    int editWalletInfoDri(UserWallet userWallet);

    /**
     * 更改用户账户余额
     * @param userWalletInfo 用户钱包信息
     * @return 1/0
     */
    int editWalletBcardInfo(UserWallet userWalletInfo);
}
